import type { AnalysisResult, TaskStatusResponse } from "@/types/analysis";
	...
	if (!response.ok) {
		...
		if (response.status === 502 || response.status === 503) {
			const error = new ExpectedError(
				errorMessage || "Сервер недоступен. Проверьте, запущен ли backend.",
			);
		...
		if (response.status === 413) {
			const error = new ExpectedError(
				errorMessage || "Файл слишком большой. Максимальный размер: 200MB.",
			);
			(error as any).statusCode = response.status;
			throw error;
		}
	...
	return {
		...
		confidence_index: {
			...
			components: {
				...
				volume_label:
					components?.volume_label ?? components?.volume_score_label ?? "",
				...
				gesture_score: Math.min(
					100,
					Math.max(0, components?.gesture_score ?? 0),
				),
				gesture_label:
					components?.gesture_label ?? components?.gesture_advice ?? "",
				gesture_advice:
					components?.gesture_advice ?? components?.gesture_label ?? "",
		...
		speech_report: {
			...
			useful_links: Array.isArray(speechReport.useful_links)
		...
		transcribe_model:
			payload?.transcribe_model ?? payload?.transribe_model ?? "",
	};
}
...
export async function uploadVideo(
	...
	formData.append(
		"user_need_text_from_video",
		String(payload.userNeedTextFromVideo),
	);
	formData.append(
		"user_need_video_analysis",
		String(payload.userNeedVideoAnalysis),
	);
...
export function resolveVideoUrl(videoPath: string): string {
	...
	if (videoPath.startsWith("/") && !videoPath.startsWith("/media/"))
		return videoPath;
