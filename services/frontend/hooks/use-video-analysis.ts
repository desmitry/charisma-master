...
import {
	...
	getFastRequestsCount,
	hasFastRequestsAvailable,
...
export function useVideoAnalysis() {
	...
	const [selectedEvaluationPreset, setSelectedEvaluationPreset] = useState<
		"default" | "urfu"
	>("default");
	const [evaluationCriteriaFile, setEvaluationCriteriaFile] =
		useState<File | null>(null);

	const [selectedPersona, setSelectedPersona] = useState<string>(
		"speech_review_specialist",
	);
	const [selectedAnalyzeProvider, setSelectedAnalyzeProvider] =
		useState<string>("gigachat");
	const [selectedTranscribeProvider, setSelectedTranscribeProvider] =
		useState<string>("sber_gigachat");
	...
	const validateRuTubeUrl = (url: string): boolean => {
		...
		try {
			...
			return (
				hostname.includes("rutube.ru") && urlObj.pathname.includes("/video/")
			);
		} catch {
			return false;
		}
	...
	const hasSpeechVideo =
		Boolean(speechVideoFile) || Boolean(speechVideoUrl && isValidRuTubeUrl);
	...
	const handleAnalyze = async () => {
		...
		try {
			...
			const userNeedTextFromVideo =
				inputMode === "speech_video" ? needTextFromVideo : false;
			const userNeedVideoAnalysis = needVideoAnalysis;
			const evaluationCriteriaId =
				criteriaMode === "preset" ? selectedEvaluationPreset : undefined;
			const criteriaFile =
				criteriaMode === "custom" ? evaluationCriteriaFile : null;

			if (
				userNeedTextFromVideo &&
				selectedTranscribeProvider === "whisper_openai"
			) {
