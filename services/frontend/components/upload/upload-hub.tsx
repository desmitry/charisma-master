...
import { StepperContent, StepperIndicatorRow } from "@/components/ui/stepper";
import type {
	useVideoAnalysis,
	WizardStepId,
} from "@/hooks/use-video-analysis";
...
const STEPS: StepDefinition[] = [
	{
		...
		description:
			"Загрузите файл с текстом речи или видеозапись — текст будет извлечён автоматически.",
	},
	{
		id: "presentation",
		title: "Презентация",
		description:
			"Добавьте презентацию, чтобы анализировать речь вместе со слайдами.",
		optional: true,
	},
	{
		id: "video",
		title: "Видеоанализ",
		description:
			"Нужен ли анализ невербальной подачи? Если видео ещё не добавлено, загрузите его здесь.",
		optional: true,
	},
	{
		id: "criteria",
		title: "Критерии",
		description:
			"Загрузите свои критерии, выберите готовый пресет или пропустите шаг.",
		optional: true,
	},
	{
		id: "settings",
		...
		description:
			"Выберите персону оценки и сервисы для анализа и транскрибации.",
		optional: true,
	},
	{
		id: "review",
...
const PERSONAS = [
	{
		id: "speech_review_specialist",
		label: "Специалист",
		description: "Нейтральная базовая оценка выступления.",
	},
	{
		id: "strict_critic",
		label: "Строгий критик",
		description: "Жёстче подсвечивает недочёты и упущения.",
	},
	{
		id: "kind_mentor",
		label: "Поддерживающий ментор",
		description: "Даёт мягкий и развивающий фидбэк.",
	},
	{
		id: "steve_jobs_style",
		label: "Визионер",
		description: "Смотрит на подачу через призму харизмы и вовлечения.",
	},
];
		...
		<div className="group/field rounded-2xl border border-white/[0.08] bg-gradient-to-b from-white/[0.04] to-white/[0.015] p-5 transition-colors hover:border-white/[0.14]">
			<div className={cn("flex items-start gap-3", file ? "mb-3" : "mb-4")}>
				...
				<div className="min-w-0">
					...
					<p className="mt-0.5 text-[13px] leading-5 text-white/35">
						{description}
					</p>
				</div>
			</div>
...
function SummaryRow({ label, value }: { label: string; value: string }) {
	return (
		<div className="flex flex-col gap-1 rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3">
			<span className="text-[11px] font-mono uppercase tracking-[0.15em] text-white/25">
				{label}
			</span>
...
export function UploadHub({ videoAnalysis }: UploadHubProps) {
	...
	const stepCompletionMap: Record<WizardStepId, boolean> = {
		...
		criteria:
			...
			(state.criteriaMode === "custom" &&
				Boolean(state.evaluationCriteriaFile)),
		settings: Boolean(
			state.selectedPersona &&
				state.selectedAnalyzeProvider &&
	...
	const reviewPayload = [
		state.inputMode === "speech_text" && state.speechTextFile
			...
			: null,
		state.speechVideoFile
			? { key: "user_speech_video_file", value: state.speechVideoFile.name }
			: null,
		state.speechVideoUrl
			? { key: "user_speech_video_url", value: state.speechVideoUrl }
			: null,
		{
			key: "user_need_text_from_video",
			value: String(
				state.inputMode === "speech_video" ? state.needTextFromVideo : false,
			),
		},
		{ key: "user_need_video_analysis", value: String(state.needVideoAnalysis) },
		state.presentationFile
			? { key: "user_presentation_file", value: state.presentationFile.name }
			: null,
		state.criteriaMode === "custom" && state.evaluationCriteriaFile
			? {
					key: "evaluation_criteria_file",
					value: state.evaluationCriteriaFile.name,
				}
			: null,
		state.criteriaMode === "preset"
			? { key: "evaluation_criteria_id", value: state.selectedEvaluationPreset }
			: null,
					...
					>
						...
						<div className="rounded-2xl border border-white/[0.08] bg-gradient-to-b from-white/[0.04] to-white/[0.015] p-5">
							...
							<input
								...
								onChange={(e) =>
									actions.handleSpeechVideoUrlChange(e.target.value)
								}
							...
							{state.speechVideoUrl && !state.isValidRuTubeUrl && (
								<p className="mt-2 text-xs text-amber-300/70">
									Ссылка не похожа на корректную ссылку RuTube.
								</p>
							)}
						</div>
	...
	const renderPresentationStep = () => (
		<div className="space-y-5">
			...
			<NavFooter>
				...
				<NextButton
					onClick={goNext}
					label={state.presentationFile ? "Дальше" : "Пропустить"}
				/>
				<PrimaryButton
					onClick={() => {
						setDirection(1);
	...
	const renderVideoStep = () => {
		const uploadedTextNotVideo =
			state.inputMode === "speech_text" && !state.hasSpeechVideo;
						...
						>
							{state.hasSpeechVideo ? (
								<div className="flex items-center gap-3 rounded-xl border border-emerald-400/15 bg-emerald-400/[0.05] px-4 py-3">
									...
									<p className="text-[13px] leading-5 text-emerald-100/70">
										Видео уже добавлено. Можно заменить ниже.
									</p>
								</div>
							) : (
								<div className="flex items-center gap-3 rounded-xl border border-amber-400/15 bg-amber-400/[0.05] px-4 py-3">
									...
									<p className="text-[13px] leading-5 text-amber-100/70">
										Видео не добавлено. Загрузите файл или ссылку.
									</p>
								</div>
							)}
					...
					>
						<div className="rounded-2xl border border-white/[0.08] bg-gradient-to-b from-white/[0.04] to-white/[0.015] p-5">
							...
							<select
								...
								onChange={(e) =>
									actions.setSelectedEvaluationPreset(
										e.target.value as "default" | "urfu",
									)
								}
		...
		<div className="space-y-5">
			<div className="grid gap-3 sm:grid-cols-2">
				...
				<SummaryRow
					label="Транскрибация"
					value={
						state.inputMode === "speech_video" && state.needTextFromVideo
							? "Да"
							: "Нет"
					}
				/>
				<SummaryRow
					label="Презентация"
					value={state.presentationFile ? state.presentationFile.name : "—"}
				/>
				<SummaryRow
					label="Видеоанализ"
				...
				<SummaryRow
					...
					value={
						state.criteriaMode === "custom"
							...
							: state.criteriaMode === "preset"
								? (PRESETS.find((p) => p.id === state.selectedEvaluationPreset)
										?.label ?? state.selectedEvaluationPreset)
								: "—"
					}
				/>
				<SummaryRow
					label="Персона"
					value={
						PERSONAS.find((p) => p.id === state.selectedPersona)?.label ??
						state.selectedPersona
					}
				/>
				<SummaryRow
			...
			<div className="rounded-2xl border border-white/[0.06] bg-black/20 p-5">
				...
				<div className="space-y-1.5">
					{reviewPayload.map((item) => (
						...
						>
							<code className="text-xs text-white/35 font-mono">
								{item.key}
							</code>
	...
	return (
		<section
			id="upload-hub"
			className="relative z-10 mb-32 w-full px-4 py-16 sm:px-6"
		>
