...
export function ConfidenceGauge({ data }: { data: ConfidenceIndex }) {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-3xl border border-foreground/10 bg-foreground/5 p-4">
      <p className="text-sm text-foreground/70">Индекс уверенности</p>
      <div className="flex flex-col gap-6 xl:flex-row xl:items-center">
        <div className="relative h-36 w-36">
          <svg viewBox="0 0 200 200" className="h-full w-full -rotate-90">
            <circle cx="100" cy="100" r={radius} stroke="var(--ring-track)" strokeWidth={stroke} fill="none" />
            <circle
              cx="100"
              cy="100"
              r={radius}
          ...
          <div className="absolute inset-0 flex items-center justify-center rotate-90">
            <div className="text-center">
              <p className="text-3xl font-semibold text-foreground">{norm}%</p>
              <p className="text-xs text-foreground/60">{data.total_label}</p>
            </div>
          </div>
        </div>
        <div className="grid flex-1 grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {metrics.map((metric) => (
            <div key={metric.label} className="rounded-2xl border border-foreground/10 bg-foreground/5 p-3">
              <p className="text-xs uppercase tracking-[0.2em] text-foreground/50">{metric.label}</p>
              <p className="mt-1 text-lg font-semibold text-foreground">{Math.round(metric.value)}%</p>
              <p className="mt-1 text-xs text-foreground/50">{metric.meta}</p>
            </div>
          ))}
        </div>
      </div>
