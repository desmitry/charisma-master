...
import {
	VideoPlayer,
	type VideoPlayerRef,
} from "@/components/shared/video-player";
	...
	return (
		<motion.div
			...
			className={cn(
				...
				className,
				accent ?? "border-white/[0.07] bg-[#0f0f0f]",
			)}
		>
			<button
				onClick={() => setOpen((v) => !v)}
...
export function AnalysisDashboard({ result, onBack }: Props) {
	...
	const [videoSrc, setVideoSrc] = useState(() =>
		result.video_path ? resolveVideoUrl(result.video_path) : "",
	);
	...
	const groupedTranscript = useMemo(() => {
		...
		const allPauses: TimelineItem[] = result.long_pauses.map((pause) => ({
			type: "pause" as const,
			pause,
		}));
	...
	const criteriaPercent =
		evaluationReport.max_score > 0
			? Math.round(
					(evaluationReport.total_score / evaluationReport.max_score) * 100,
				)
			: 0;
			...
			? [
					{
						...
						extra: `${result.fillers_summary.count} шт. · ${(result.fillers_summary.ratio * 100).toFixed(1)}%`,
					},
				]
			: []),
		...(needVideoAnalysis
	...
	const aiCards = [
		{
			...
			text: speechReport.summary,
			icon: <FileText className="w-4 h-4 text-emerald-400" />,
		},
		{
			title: "Структура",
			text: speechReport.structure,
			icon: <ListTree className="w-4 h-4 text-amber-400" />,
		},
		{
			title: "Ошибки",
			text: speechReport.mistakes,
			icon: <AlertTriangle className="w-4 h-4 text-rose-400" />,
		},
		{
			title: "Идеальный текст",
			text: speechReport.ideal_text,
			icon: <Wand2 className="w-4 h-4 text-fuchsia-400" />,
		},
		{
			title: "Фидбэк персоны",
			text: speechReport.persona_feedback,
			icon: <User className="w-4 h-4 text-blue-400" />,
		},
		{
			title: "Фидбэк по презентации",
			text: speechReport.presentation_feedback,
			icon: <Presentation className="w-4 h-4 text-indigo-400" />,
		},
		...(speechReport.useful_links.length > 0
			? [
					{
						title: "Полезные ссылки",
						text: speechReport.useful_links.join("\n"),
	...
	// avg wpm
	const avgWpm =
		needTranscript && result.tempo.length
			? Math.round(
					result.tempo.reduce((s, p) => s + p.wpm, 0) / result.tempo.length,
				)
			: 0;
	...
	const statPills = [
			...
			? [
					{
						...
						value: Math.round(confidence.total),
						sub: confidence.total_label,
					},
				]
			: []),
		...(needTranscript
			...
			: []),
		{
			...
			sub: `${evaluationReport.total_score}/${evaluationReport.max_score}`,
		},
	];
	...
	const hasRightCol =
		needTranscript || needVideoAnalysis || evaluationReport.criteria.length > 0;

	return (
		<div
			className="relative min-h-screen bg-[#080808] text-white w-full selection:bg-white/10"
				...
				<div className="mx-auto flex w-full max-w-[1280px] items-center justify-between px-5 py-3.5 gap-4">
					<div className="flex items-center gap-4 min-w-0">
						<div className="flex flex-col gap-1 min-w-0">
							<h1 className="text-[14px] font-semibold text-white/90 truncate">
								Разбор выступления
							</h1>
							{metadataChips.length > 0 && (
								<div className="hidden sm:flex items-center gap-1.5 flex-wrap">
									{metadataChips.map((c) => (
										<span
											key={c.label}
											className="text-[10px] font-mono text-white/30 px-2 py-0.5 rounded bg-white/[0.04] border border-white/[0.06]"
										>
											{c.label}: {c.value}
										</span>
									))}
					...
					{activeTab === "overview" && (
						...
						>
							...
							>
								...
								<ActivityRingsCard
									result={result}
									needVideoAnalysis={needVideoAnalysis}
									needTranscript={needTranscript}
								/>
							</motion.div>
							...
							{statPills.length > 0 && (
								...
								>
									{statPills.map((pill) => (
										<div
											key={pill.label}
											className="rounded-2xl border border-white/[0.06] bg-[#0f0f0f] p-3.5 sm:px-5 sm:py-4"
										>
											<StatPill
												label={pill.label}
												value={pill.value}
												...
												badge={
													hasVideo ? (
														<span className="text-[10px] font-mono text-white/25 ml-1">
															нажмите на слово для перемотки
														</span>
													) : undefined
												}
											>
														...
														<div key={gi} className="mb-5 last:mb-0">
															...
															<div className="text-[14px] leading-[1.9] text-white/65">
																{group.items.map((item, ii) => {
																	if (item.type === "pause") {
																		const isActive =
																			currentTime + 0.02 >= item.pause.start &&
																			currentTime < item.pause.end - 0.02;
																		return (
																			<span
																	...
																	const isActive =
																		currentTime + 0.02 >= item.word.start &&
																		currentTime < item.word.end - 0.02;
											...
											<Accordion
												...
												badge={
													<span className="text-[10px] font-mono text-white/30 border border-white/[0.08] px-2 py-0.5 rounded-full">
														{result.fillers_summary.count} ·{" "}
														{(result.fillers_summary.ratio * 100).toFixed(1)}%
													</span>
												}
											>
												{fillerWordsList.length > 0 ? (
													<div className="flex flex-wrap gap-2">
														{fillerWordsList.map(([word, count]) => (
															<span
																key={word}
																className="inline-flex items-center gap-2 rounded-full bg-white/[0.05] border border-white/[0.08] px-3 py-1 text-xs text-white/70"
															>
																{word}
						...
						>
							...
							{speechReport.summary && (
								...
								>
									...
									<p className="text-[15px] leading-[1.8] text-white/75 whitespace-pre-line">
										{speechReport.summary}
									</p>
								</motion.div>
							)}
							...
							{needTranscript && speechReport.dynamic_fillers.length > 0 && (
								...
								>
									<div className="text-[10px] font-mono uppercase tracking-[0.2em] text-white/30 mb-3">
										Слова-паразиты, замеченные ИИ
									</div>
									<div className="flex flex-wrap gap-2">
										{speechReport.dynamic_fillers.map((word, i) => (
											<span
												key={`${word}-${i}`}
												className="inline-flex items-center rounded-full border border-white/[0.08] bg-white/[0.04] px-3 py-1 text-sm text-white/65"
											>
												{word}
											</span>
										))}
					...
					{activeTab === "criteria-report" && (
						...
						>
							...
							<motion.div
								...
								className="rounded-2xl border border-white/[0.07] bg-[#0f0f0f] p-6"
							>
								<div className="text-[10px] font-mono uppercase tracking-[0.2em] text-white/30 mb-5">
									Итоговый результат
								</div>
							...
							{evaluationReport.criteria.length > 0 ? (
								<div className="flex flex-col gap-3">
									{evaluationReport.criteria.map((crit, i) => {
										...
										const pct =
											crit.max_value > 0
												? Math.round((val / crit.max_value) * 100)
												: 0;
										return (
											<Accordion
												key={`${crit.name}-${i}`}
												...
												<div className="flex flex-col gap-3">
													...
													{crit.feedback && (
														<div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3">
															<div className="text-[10px] font-mono uppercase tracking-[0.15em] text-white/25 mb-1.5">
																Совет
															</div>
								...
								<motion.div
									variants={fadeUp}
									className="rounded-2xl border border-white/[0.07] bg-[#0f0f0f] p-6 text-sm text-white/40"
								>
