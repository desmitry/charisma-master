...
import type { TempoPoint } from "@/types/analysis";
	...
	if (active && payload && payload.length) {
		...
		return (
			<div className="max-w-[260px] rounded-lg bg-black/95 border border-white/10 px-3 py-2 text-xs shadow-lg">
				...
				<div className="mt-1 text-white/60">{getTempoMeaning(point.wpm)}</div>
			</div>
		);
	}
	return null;
...
export function TempoChart({ data, currentTime }: Props) {
	...
	const yRange = useMemo(() => {
		...
		const pad = range * 0.1;
		return [
			Math.max(0, Math.floor(stats.min - pad)),
			Math.ceil(stats.max + pad),
		];
	}, [stats]);
							...
							>
								{data.map((point) => (
									<Cell
										...
										fill={getZoneMeta(point.zone).color}
									/>
								))}
							</Bar>
						</BarChart>
