...
function ChevronIcon({ open }: { open: boolean }) {
  return (
    <svg
      ...
      className={cn("flex-shrink-0 text-white/25 transition-transform duration-300", open ? "rotate-180" : "")}
    >
    ...
    <div className="border-b border-white/[0.04] last:border-0">
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="text-[13px] text-white/75 font-medium truncate">{criterion.name}</div>
        </div>
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <div className="hidden sm:flex items-center gap-2">
            <div className="w-16 h-1 rounded-full bg-white/[0.06] overflow-hidden">
              <div
                className={cn("h-full rounded-full", pct >= 70 ? "bg-white/55" : pct >= 40 ? "bg-white/30" : "bg-white/12")}
                style={{ width: `${pct}%`, transition: "width 0.8s ease" }}
              />
            </div>
          </div>
          <span className="text-[12px] font-mono text-white/40 whitespace-nowrap">
            {val}<span className="text-white/20">/{criterion.max_value}</span>
          </span>
          ...
          >
            <div className="px-4 pb-4 flex flex-col gap-3">
              ...
              <div className="h-1.5 w-full rounded-full bg-white/[0.05] overflow-hidden">
                <div
                  className={cn("h-full rounded-full transition-all duration-700", pct >= 70 ? "bg-white/55" : pct >= 40 ? "bg-white/30" : "bg-white/12")}
                  style={{ width: `${pct}%` }}
                />
              </div>
              {criterion.description && (
                <p className="text-[12px] leading-relaxed text-white/40">{criterion.description}</p>
              )}
              {criterion.feedback && (
                <div className="rounded-xl border border-white/[0.05] bg-white/[0.02] px-3 py-2.5">
                  <div className="text-[9px] font-mono uppercase tracking-[0.15em] text-white/20 mb-1">Совет</div>
...
export function StandardCriteriaTable({ report }: Props) {
  ...
  return (
    <motion.div variants={fadeUpAnim} className="rounded-2xl border border-white/[0.07] bg-[#0f0f0f] overflow-hidden">
      {/* header */}
      <div className="px-4 py-3.5 border-b border-white/[0.04] flex flex-row items-center justify-between gap-3">
        <span className="text-[11px] font-mono uppercase tracking-[0.18em] text-white/35">Оценки по критериям</span>
        <span className={cn(
          "text-[11px] font-mono px-2 py-0.5 rounded-full border",
          pct >= 70 ? "text-white/60 border-white/15 bg-white/[0.04]" : pct >= 40 ? "text-white/45 border-white/10 bg-white/[0.03]" : "text-white/25 border-white/[0.07] bg-white/[0.02]"
        )}>
