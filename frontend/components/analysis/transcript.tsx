...
import { TranscriptSegment, LongPause } from "@/types/analysis";
...
export function Transcript({
  ...
  const groupedItems = useMemo(() => {
    const groups: { label: string; segmentStart: number; items: TimelineItem[] }[] = [];
    const chunkSize = 30;
    ...
    timeline.forEach((item) => {
      ...
      if (!groups[groupIdx]) {
        groups[groupIdx] = { label, segmentStart: groupIdx * chunkSize, items: [] };
      }
        ...
        >
          ...
          <div className="mt-1.5 text-[13px] sm:text-[14px] leading-[1.5] sm:leading-[1.4] break-words overflow-wrap-anywhere">
            {group.items.map((item, idx) => {
              if (item.type === "pause") {
                const isActive =
                  currentTime + eps >= item.pause.start && currentTime < item.pause.end - eps;
                return (
                  <span
              ...
              const isActive =
                currentTime + eps >= item.word.start && currentTime < item.word.end - eps;
              return (
                <span
