...
export function Particles({
  ...
  useEffect(() => {
    ...
    const handleMouseDown = () => {
      ...
      if (hit) {
        ...
        spawnWave(new THREE.Vector2(currentMouseWorld.current.x, currentMouseWorld.current.z));
      }
    };
  ...
  useFrame((state, delta) => {
    ...
    simulationMaterial.uniforms.uMouseActive.value = isMouseOver.current ? 1.0 : 0.0;
    ...
    const simWaveOrigins =
      simulationMaterial.uniforms.uWaveOrigins.value as THREE.Vector2[];
    const simWaveProgress =
      simulationMaterial.uniforms.uWaveProgress.value as number[];
    const simWaveActive =
      simulationMaterial.uniforms.uWaveActive.value as number[];
    ...
    dofPointsMaterial.uniforms.uMouseActive.value = isMouseOver.current ? 1.0 : 0.0;
    ...
    const pointWaveOrigins =
      dofPointsMaterial.uniforms.uWaveOrigins.value as THREE.Vector2[];
    const pointWaveProgress =
      dofPointsMaterial.uniforms.uWaveProgress.value as number[];
    const pointWaveActive =
      dofPointsMaterial.uniforms.uWaveActive.value as number[];
