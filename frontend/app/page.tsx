          ...
          <div className="flex flex-col items-center justify-center px-4 py-16 sm:px-6 lg:min-h-svh">
            <div className="w-full max-w-4xl rounded-[32px] border border-white/10 bg-black/35 px-6 py-12 shadow-[0_40px_120px_rgba(0,0,0,0.45)] backdrop-blur-xl sm:px-8 sm:py-14">
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <div className="rounded-3xl border border-white/15 bg-white/5 px-6 py-6">
                ...
                <div className="rounded-3xl border border-white/15 bg-white/5 px-6 py-6">
                  ...
                  <div ref={providerRef} className="relative">
                    <button
                      ...
                      onClick={() => {
                        if (!isProviderOpen && providerButtonRef.current) {
                          ...
                          const spaceAbove = rect.top;
                          const dropdownHeight = 200;
                          setProviderOpenUp(spaceBelow < dropdownHeight && spaceAbove > spaceBelow);
                        }
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          {selectedLlmProvider === "gigachat" ? "GigaChat" :
                      ...
                      <svg
                        className={cn(
                          "w-4 h-4 text-white/60 transition-transform duration-300 ease-in-out",
                          isProviderOpen ? "rotate-180" : "rotate-0"
                        )}
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="2"
                    ...
                    <div
                      className={cn(
                        "absolute z-50 w-full rounded-2xl border border-white/20 bg-black/40 backdrop-blur-xl overflow-hidden transition-all duration-300",
                        providerOpenUp ? "bottom-full mb-2" : "top-full mt-2",
                        isProviderOpen
                          ? "opacity-100 translate-y-0 pointer-events-auto"
                          : providerOpenUp
                          ? "opacity-0 translate-y-2 pointer-events-none"
                      ...
                      style={{
                        maxHeight: isProviderOpen ? "300px" : "0px",
                        transition: "opacity 300ms cubic-bezier(0.4, 0, 0.2, 1), transform 300ms cubic-bezier(0.4, 0, 0.2, 1), max-height 300ms cubic-bezier(0.4, 0, 0.2, 1)"
                      }}
                    >
                      <div className="py-1">
                        {[
                          { value: "gigachat", label: "GigaChat", desc: "Sber GigaChat (GigaChat-2)" },
                          { value: "openai", label: "OpenAI", desc: "OpenAI GPT (gpt-3.5-turbo)" }
                        ].map((option) => (
                          <button
                            key={option.value}
                            type="button"
                  ...
                  <p className="mt-3 text-xs text-white/50">
                    {
                      selectedLlmProvider === "gigachat" ? "Российская модель от Сбера" :
                      selectedLlmProvider === "openai" ? "Модель от OpenAI" :
